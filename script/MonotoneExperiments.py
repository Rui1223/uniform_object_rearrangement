...
def main(args):
    ...
    for num_objects in monotone_experimenter.numObjects_options:
        ...
        for experiment_id in range(1, monotone_experimenter.numExperiments_perObject+1):
            ...
            if (num_monotoneInstancesSaved >= max_instances_need): break
            ### generate the instance in the execution scene
            ...
            TASK_SUCCESS = rearrangement_task_planner.DFS_DP_task_planning(len(cylinder_objects))
            DFS_DP_labeled_planning_time = time.time() - start_time
            print("Time for DFS_DP planning is: {}".format(DFS_DP_labeled_planning_time))

            ### (ii) DFS_DP
            # start_time = time.time()
            # TASK_SUCCESS = rearrangement_task_planner.DFS_DP_task_planning(len(cylinder_objects), isLabeledRoadmapUsed=False)
            # DFS_DP_planning_time = time.time() - start_time
            # print("Time for DFS_DP planning is: {}".format(DFS_DP_planning_time))

            ### (iii) mRS_labeled
            # start_time = time.time()
            # TASK_SUCCESS = rearrangement_task_planner.mRS_task_planning(len(cylinder_objects))
            # mRS_labeled_planning_time = time.time() - start_time
            # print("Time for mRS planning is: {}".format(mRS_labeled_planning_time))

            ### (iv) mRS
            # start_time = time.time()
            # TASK_SUCCESS = rearrangement_task_planner.mRS_task_planning(len(cylinder_objects), isLabeledRoadmapUsed=False)
            # mRS_planning_time = time.time() - start_time
            # print("Time for mRS planning is: {}".format(mRS_planning_time))
            #######################################################################################################

            if TASK_SUCCESS:
                monotone_experimenter.saveInstance(num_objects, experiment_id, cylinder_objects)
                num_monotoneInstancesSaved += 1
                # input("enter to start the execution!!!!!")
                # start_time = time.time()
                # execute_success = monotone_experimenter.executeWholePlan(rearrangement_task_planner.object_paths)
